...
class ConnectionManager:
    ...
    async def broadcast(self, session_id: str, message: dict):
        if session_id in self.active_connections:
            # 방에 있는 모든 클라이언트(보호자/피보호자)에게 데이터 중계
...
ROUTE_CACHE_TTL_SECONDS = 60 * 60  # 1시간
...
async def websocket_endpoint(
    ...
    """
    클라이언트(보호자/피보호자)가 session_id 채널로 연결하여 실시간 위치 수발신
    
    [피보호자 전송 JSON 데이터 전송 규격 예시]
    {
    ...
    try:
        while True:
            ...
            if data.get("type") == "LOCATION_UPDATE":
                ...
                # 2) 방 안에 있는 모든 연결(보호자 앱 등)로 실시간 좌표 브로드캐스팅
                broadcast_payload = {
                    "session_id": session_id,
